    ...
    return (
        <ListGroup.Item className="d-flex flex-row align-items-center m-4 border rounded-pill sticky-top" style={{'background-color':'#3e3f48'}}>
            ...
            <Form onSubmit={handleSubmit}>
                <Form.Control className='rounded-pill border-0 shadow-0' style={{'box-shadow':'none', 'background-color':'#3e3f48', fontFamily:'Roboto Condensed, sans-serif'}} value={search} placeholder="Cerca" onChange={(e) => { setSearch(e.target.value) }} onFocus={() => setSearchIcon(!searchIcon)} />
            </Form>
        </ListGroup.Item>
    )
}
