...
export default function SearchBar({ search, setSearch }) {
    ...
    return (
        <ListGroup.Item className="d-flex flex-row align-items-center m-4 border rounded-pill sticky-top" style={{'background-color':'#3e3f48'}}>
            ...
            <Form>
                <Form.Control className='rounded-pill border-0 shadow-0' style={{'box-shadow':'none', 'background-color':'#3e3f48'}} value={search} placeholder="Cerca" onChange={(e) => { setSearch(e.target.value) }} onFocus={() => setSearchIcon(!searchIcon)} />{/*Modifica il filtro*/}
            </Form>
        </ListGroup.Item>
    )
}
